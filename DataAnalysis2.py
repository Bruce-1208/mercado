...
def getToken(refresh):
    url = (
        "https://api.mercadolibre.com/oauth/token?grant_type=refresh_token&client_id="
        + id
        + "&client_secret="
        + pwd
        + "&refresh_token="
...
headers = {
    "Authorization": "Bearer APP_USR-8820539028080485-112005-52f5c064adaa4bcf20e33c8da358b73a-1742669993"
}
...
def getAtribute(cbtid, dictItem):
    url = (
        ...
        + cbtid
        + "&attributes=id,price,category_id,title,domain_id,pictures,status,date_created,listing_type_id"
    )
    rep = fetch_with_retries(url)
...
def getVisit(item):
    ...
    if status != "active":
        ...
        return
    url = (
        "https://api.mercadolibre.com/items/visits?ids="
        + item_id
        + "&date_from="
        + start_date
        + "&date_to="
...
def main():
    ...
    url = (
        ...
        + userid
        + "/items/search?search_type=scan&limit=100&order=start_time_asc"
    )
    ##获取所有所有站点的cbt编吗
    rep = fetch_with_retries(url)
    cbtlist = []
