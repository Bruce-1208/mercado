...
汇率、分类佣金和各站点运费报价使用 24 小时 MySQL 缓存，商品列表和批量上架
优先读取缓存，不会逐件重复请求官网最新值：

- `erp_mercadolibre_exchange_rates`：六个可选站点货币兑 USD 的每日汇率；
- `erp_mercadolibre_category_commissions`：按站点、分类、刊登类型、价格、物流和
  计费重量保存的佣金报价；
- `erp_mercadolibre_shipping_rates`：按站点、卖家、分类、价格和包裹参数保存的
  全国运费报价。

后台每小时检查一次过期时间，但同一缓存记录只有超过 24 小时才会调用官网刷新；
商品费用快照同样每天刷新一次。
