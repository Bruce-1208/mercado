...
链接和采集数量。默认使用 Playwright 直接读取页面 DOM：列表页自动翻页，再通过
常驻页面池并发补充详情。标题、原价、主图和来源从列表卡片读取；重量和三边尺寸直接
读取智赢插件已在列表页批量加载的 React 数据，不操作鼠标键盘，也不会读取智赢
产品库。详情页补充描述、规格和图片，并尝试补齐列表缺失的插件数据。
列表插件首次加载保留默认 60 秒预算；已有完成响应后，连续 10 秒没有数据变化便
继续采集，避免个别未返回的商品拖住整页。可通过
`MERCADO_PLAYWRIGHT_LISTING_PLUGIN_IDLE_SECONDS` 调整此等待时间；设置为 60
可恢复等待到整页总超时的行为。插件连接不可用时立即进入后续流程；卡住的 CDP
读取也受整页超时限制。详情页跳转到买家验证地址时立即进入原有冷却流程。
