...
def check_yuanyou_title(number):
    ...
    while n < page:
        ...
        try:
            ...
            isSuccess=True
            for title in titles:
                # 假设你已经拿到了标题元素 title_element
                # <div class="f12 product-title">Adaptador De Tomada...</div>

                # 1. 连续向上找两层父级，定位到最外层的 product-item
                product_item_element = title.find_element(By.XPATH, "./..")  # 第一层是 product-info
                main_card_element = product_item_element.find_element(By.XPATH, "./..")  # 第二层是 product-item

                # 2. 从最外层卡片向下直接定位到图片标签
                img_element = main_card_element.find_element(By.CSS_SELECTOR, "img.product-pic")
...
def up1688(img_url):
    ...
    # ---- 步骤 1: 下载美客多图片 ----
    img_url = "https://http2.mlstatic.com/D_659211-CBT110453715415_042026-F.jpg"
    img_path= img_url.split("/")[-1]

    local_img_path = os.path.abspath(img_path)
    ...
    if response.status_code == 200:
        ...
        print("✓ 图片成功下载到本地:", local_img_path)

    # ---- 步骤 2: 启动防检测浏览器 ----
    options = uc.ChromeOptions()
    # options.add_argument('--headless') # 如果需要无头模式可以开启
    ...
    try:
        ...
        driver.find_element(By.ID, "alisearch-input")
        global_file_xpath = "//input[@type='file']"

        # 3. 直接强行塞入路径
        upload_node = driver.find_element(By.XPATH, global_file_xpath)
        upload_node.send_keys(local_img_path)

        print("✓ 已将图片路径提交给1688，等待页面跳转...")

        # ---- 步骤 4: 等待搜索结果 ----
        # 上传后页面会自动处理并跳转，通常需要 5-10 秒
        time.sleep(8)

        # 打印当前跳转后的 URL，确认是否进入了搜货结果页
        print("当前页面URL:", driver.current_url)

        # 在这里可以继续编写你解析 1688 货源列表的代码...
...
if __name__ == '__main__':
    ...
    url="https://http2.mlstatic.com/D_736518-CBT111322350873_052026-F.jpg"
    up1688(url)
