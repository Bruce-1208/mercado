...
def download_excel(driver, mail_item):
    ...
    time.sleep(10)
    contain = wait.until(
        EC.presence_of_element_located((By.CLASS_NAME, "wide-content-host"))
    )

    element_download = wait.until(
        EC.presence_of_element_located(("link text", "Go to download report"))
    )
    downlod_url = contain.find_element(
        "link text", "Go to download report"
    ).get_attribute("href")
