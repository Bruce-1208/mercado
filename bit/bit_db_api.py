...
def get_mercado_token_authorization_info(application_id=None):
    ...
    if DB_MODE == "mysql":
        ...
        application = bit_mysql.get_mercado_application(application_id)
        if not application:
            raise KeyError("开发者应用不存在")
        return authorization_info(application)
    return _request(
        "GET", f"/api/db/mercado-applications/{application_id}/authorization"
    )


def list_mercado_applications():
    if DB_MODE == "mysql":
        return _local_call("list_mercado_applications")
    return _request("GET", "/api/db/mercado-applications")


def create_mercado_application(record):
    if DB_MODE == "mysql":
        return _local_call("create_mercado_application", record or {})
    return _request("POST", "/api/db/mercado-applications", json=record or {})


def update_mercado_application(application_id, changes):
    if DB_MODE == "mysql":
        return _local_call(
            "update_mercado_application", int(application_id), changes or {}
        )
    return _request(
        "PATCH",
        f"/api/db/mercado-applications/{int(application_id)}",
        json=changes or {},
    )
...
def delete_mercado_application(application_id):
    if DB_MODE == "mysql":
        return _local_call("delete_mercado_application", int(application_id))
    return _request("DELETE", f"/api/db/mercado-applications/{int(application_id)}")
...
def reauthorize_mercado_store_application(token_id, application_id, callback_or_code):
    token_id = int(token_id)
    application_id = int(application_id)
    if DB_MODE == "mysql":
        from bit import bit_mysql
        from bit.mercado_tokens import reauthorize_with_application

        return reauthorize_with_application(
            token_id,
            callback_or_code,
            application_id,
            get_token=bit_mysql.get_mercado_store_token,
    ...
    return _request(
        "POST",
        f"/api/db/mercado-tokens/{token_id}/application",
        timeout=60,
        json={"application_id": application_id, "code": callback_or_code},
    )
...
def get_published_mercado_product_account_ids(product_item_ids):
    item_ids = [int(value) for value in product_item_ids or []]
    payload = {"product_item_ids": item_ids}
    if DB_MODE == "mysql":
        return _collection_store_call("get_published_product_account_ids", item_ids)
    path = "/api/db/mercado-publish-records/published-account-ids"
    try:
        data = _request("POST", path, json=payload)
        return {
            int(product_id): int(token_id)
            for product_id, token_id in (data.get("account_ids") or {}).items()
        }
    except RuntimeError as exc:
        if not _collection_route_missing(exc, path):
            raise
