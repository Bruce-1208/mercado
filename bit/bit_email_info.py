...
def get_mail_info(driver, text):
    ...
    try:
        emails = wait.until(
            EC.presence_of_all_elements_located((By.CSS_SELECTOR, "[data-item-index]"))
        )
    ...
    for email in emails:
        try:
            ...
            email_time = time_element.get_attribute(
                "title"
            )  # 包含具体日期：周三 2026-04-01 21:11
            # 将数据存入集合
