...
工作台账号和店铺授权都带有稳定的 `organization_key`。企业使用独立的小写标识，例如 `acme-shop`；不要用员工姓名作为企业边界。已有 `default` 企业的账号和店铺会迁移到 `wuhan-zeshun`（武汉泽顺）。超级管理员可以按企业筛选各业务功能，企业管理员只读取本企业数据，成员角色再叠加“仅本人店铺”限制。
